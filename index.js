...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀: 
// The nested function can access internal because it is within the scope of the outer function and this would be considered a closure. The inner function can reach outside of iself to grab a variable.
...
const zooAnimals = [
  ...
  */
  ...
  function animalNames(arr){
    let displayNames = arr.map(function(inst){
      return `name: ${inst.animal_name}, scientific: ${inst.scientific_name}`
    });
  ...
  */
  ...
  function lowerCaseNames(arr){
   let newArr = arr.map((inst) => {
     return inst.animal_name.toLowerCase();
   })
   return newArr;
  }
  ...
  */
  ...
  function lowPopulationAnimals(arr){
    let newArr = arr.filter((inst) => {
      return inst.population < 5;
    })
    return newArr;
  }
  ...
  */
  ...
  function USApop(array){
    let pop = array.reduce((acc, inst) => {
      return acc + inst.population;
    },0)
    return pop;
  }
...
//They are already arrow functions!!
